...
export const StopInput = ({ variant }: { variant: "from" | "to" }) => {
    ...
    return (<Autocomplete
        ...
        rightSection={typeof selected === 'undefined' && input.length === 0 ? <></> :
            <Menu onClose={() => setMenuOpen(-1)} onOpen={() => setMenuOpen(variant === "from" ? 0 : 1)} opened={variant === "from" ? menuOpen === 0 : menuOpen === 1} position="bottom-end" styles={(theme) => ({ dropdown: { minWidth: '15rem', background: theme.colors.dark[7], borderRadius: theme.radius.md } })}>
                <Menu.Target>
