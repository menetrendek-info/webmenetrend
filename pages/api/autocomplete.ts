...
export default async function handler(
    ...
    const results = await query('SELECT c.name as city, s.id as id, CONCAT(c.name, ", ", s.name) as stop_name, network FROM stops as s LEFT JOIN cities as c ON s.city=c.id WHERE CONCAT(c.name, " ", s.name) REGEXP ? LIMIT 50;', [inp])
